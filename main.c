...
#define APP_ADV_INTERVAL_FAST               560                                       /**< Fast advertising interval (in units of 0.625 ms. This value corresponds to 25 ms.). */
#define APP_ADV_INTERVAL_SLOW               560                                       /**< Slow advertising interval (in units of 0.625 ms. This value corresponds to 2 seconds). */
...
#define BATTERY_LEVEL_MEAS_INTERVAL         APP_TIMER_TICKS(10, APP_TIMER_PRESCALER)//Tsungta, as short as possible
...
#define DIODE_FWD_VOLT_DROP_MILLIVOLTS      0//270	Tsungta, no such part in Key Ring    /**< Typical forward voltage drop of the diode (Part no: SD103ATW-7-F) that is connected in series with the voltage supply. This is the voltage drop when the forward current is 1mA. Source: Data sheet of 'SURFACE MOUNT SCHOTTKY BARRIER DIODE ARRAY' available at www.diodes.com. */
...
void saadc_event_handler(nrf_drv_saadc_evt_t const * p_event)
{
    if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
    {
        ...
        percentage_batt_lvl = battery_level_in_percent(batt_lvl_in_milli_volts);
				...
				batt_mV = batt_lvl_in_milli_volts;//Tsungta
				//percentage_batt_lvl = 50;//Tsungta, only for testing
 ...
 */
void adc_configure(void)//Tsungta
{
    #ifdef NRF51
                         ...
                         uint16_t            *  const p_len)
{
  ...
  *p_len = 0;
  uint16_t length = 0;
	...
	*p_len += uint16_inverse_encode(0x0709, &p_encoded_data[*p_len]);  // Length(including AD Type) + AD type(manufacturer)
...
static void ble_stack_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
		...
		//Atlas start
		//err_code = sd_ble_gap_tx_power_set(RADIO_TXPOWER_TXPOWER_Neg40dBm);
		//APP_ERROR_CHECK(err_code);
...
static void bsp_event_handler(bsp_event_t event)
{
    ...
    switch (event)
    {
        ...
        {
            if (m_is_ias_present && ++key0_cnt == 3)
            {
            ...
            } else err_code = ble_ias_c_send_alert_level(&m_ias_c, BLE_CHAR_ALERT_LEVEL_NO_ALERT);//Tsungta
        }break;//BSP_EVENT_KEY_0

        case BSP_EVENT_KEY_1: // STOP_ALERTING_BUTTON_ID
						key0_cnt = 0;//Tsungta
						m_is_high_alert_signalled = false;//Tsungta
            err_code = bsp_indication_set(BSP_INDICATE_ALERT_OFF);
            APP_ERROR_CHECK(err_code);
